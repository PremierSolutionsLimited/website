...
const RecentTrips: FC<ITripProps> = ({ data }) => {
  ...
  return (
    <Fragment>
      <div className="relative bg-white flex flex-col space-y-5 pt-5 px-4 pb-12 sm:pt-6 sm:px-6 shadow rounded-lg overflow-hidden">
        <div>
          <div className="flex justify-between items-top">
            ...
            <div className="flex flex-col items-center space-y-2">
              <span
                className={`inline-flex items-center px-2.5 py-0.5 rounded text-xs font-medium bg-customBlack-2 ${
        ...
        <div className="mt-4 flex justify-between text-lg ">
            <div className="text-gray-700">{new Date(data?.createdAt).toLocaleDateString()}</div>
            <h2 className="font-medium text-green-600">{data?.tripType?.name}</h2>
        </div>
        ...
        <div className="grid grid-cols-1 gap-2">
            <div className="flex justify-between text-gray-700">
                ...
                <p className="text-gold-2">{data?.startTime ? new Date(data?.startTime).toLocaleTimeString() : "Not Started Yet"}</p>
            </div>
            <div className="flex justify-between text-gray-700">
                <div className="flex">
                ...
                <p className="text-gold-2">{data?.endTime ? new Date(data?.endTime).toLocaleTimeString() : "Not Ended Yet"}</p>
            </div>
        </div>      
        <div className="absolute bottom-0 inset-x-0 bg-gray-50 p-4 ">
