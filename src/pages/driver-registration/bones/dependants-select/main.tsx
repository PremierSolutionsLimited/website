      ...
      <div style={{ height: "25vh" }} className="overflow-y-auto">
        {dependDents.map((dependent: DependentsInputProp, index: number) => (
          <React.Fragment key={index}>
            <div className="py-3 px-4 sm:p-6 lg:pb-8 shadow-sm">
              <div className="mt-2 grid grid-cols-12 relative gap-6">
                <div className={` absolute right-0`}>
                  <svg
                    onClick={() => {
                      const remainder = dependDents?.filter(
                        (_, id) => id !== index
                      );
                      setDependents(remainder);
                    }}
                    className="w-5 text-red-500 h-5 -mt-2"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
